...
LOOP_BASE=$4     # the starting point of the loop
LOOP_STEP=1000  # the step of the loop
QPS_LOOP=1

ITERATION=6     # the number of iterations of the loop
...
if [ "$NODE_CONFIG" == "local" ]; then
    sudo numactl --physcpu=12,13 --membind=$SNC_local redis-server --daemonize yes
else
